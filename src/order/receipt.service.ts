...
export class ReceiptService {
    ...
    async generateReceipt(orderId: string, userId: string, userRole: Role): Promise<Buffer> {
        ...
        if (order.status !== OrderStatus.PAID && order.status !== OrderStatus.SHIPPED) {
            throw new ForbiddenException('Struk hanya tersedia setelah pembayaran dikonfirmasi');
        }
        ...
        const docDefinition = {
            content: [
                ...
                { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1 }], margin: [0, 8, 0, 8] },

                {
                    columns: [
                        ...
                        [
                            ...
                            { text: formatDate(order.paidAt ?? order.updatedAt), style: 'value' },
                ...
                { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 0.5 }], margin: [0, 0, 0, 6] },
                {
                    columns: [
                        { text: '' },
                        {
                            ...
                            table: {
                                body: [
                                    ['Subtotal', { text: formatRupiah(order.subtotal), alignment: 'right' }],
                                    ['Ongkos Kirim', { text: formatRupiah(order.shippingFee), alignment: 'right' }],
                                    [{ text: 'TOTAL', bold: true }, { text: formatRupiah(order.total), bold: true, alignment: 'right' }],
                                ],
                            },
                            layout: 'noBorders',
                        },
            ...
            styles: {
                ...
                subheader: { fontSize: 12, alignment: 'center', color: '#666666', margin: [0, 0, 0, 8] },
